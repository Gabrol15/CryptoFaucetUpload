...
class M_Links extends CI_Model
{
	...
	public function counttotalHistory($userId)
	{
		$past = time() - 86400;
		$ip_address = $this->input->ip_address();
		return $this->db->query("SELECT COUNT(*) as cnt FROM link_history WHERE claim_time>" . $past . " AND (ip_address='" . $ip_address . "' OR user_id=" . $userId . ")")->result_array()[0]['cnt'];
	}
	...
	public function get_link_history($user_id)
	{
		$past = time() - 86400;
		$ip_address = $this->input->ip_address();
		return $this->db->query("SELECT * FROM link_history WHERE claim_time>" . $past . " AND (ip_address='" . $ip_address . "' OR user_id=" . $user_id . ")")->result_array();
	}
